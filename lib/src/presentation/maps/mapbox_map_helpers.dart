...
MapPoint defaultSevillePoint() =>
    const MapPoint(latitude: 37.3891, longitude: -5.9845);

MapPoint centerForPoints(List<MapPoint> points) {
  if (points.isEmpty) {
    return defaultSevillePoint();
  }
...
double zoomForPoints(List<MapPoint> points, {double fallback = 14}) {
  if (points.length < 2) {
    return fallback;
  }
...
CameraOptions cameraForPoints(
  ...
  final zoom = zoomForPoints(points, fallback: fallbackZoom);
